...
import { User, Asset, AssetType, Document, FamilyMember } from '../types';
import { getAssets, addAsset, deleteAsset, getDocuments, addDocument, addAuditLog, getFamilyMembers, addFamilyMember, deleteFamilyMember } from '../db';
...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ customer, actingUser }) => {
  ...
  const handleAddAsset = (newAsset: Omit<Asset, 'id' | 'lastUpdated'>) => {
    ...
    addAuditLog({ 
      ...
      action: 'ASSET_CREATE', 
      details: `Added ${asset.type} worth $${asset.value} for member ID ${asset.memberId} in ${customer.name}`, 
  ...
  return (
    <div className="space-y-10 pb-20">
      <header className="glass rounded-[40px] p-10 flex flex-col md:flex-row md:items-center justify-between gap-8 border-white/5 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 font-medium flex items-center space-x-2 uppercase tracking-widest text-[10px]">
            ...
            <span>Family Wealth Core Portal</span>
          </p>
        </div>
        <div className="flex items-center space-x-12 relative z-10">
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Net Portfolio Value</p>
          ...
          >
            <div className="flex -space-x-3 mb-3">
              ...
              {members.length > 3 && (
                <div className="w-12 h-12 rounded-2xl bg-blue-500/20 border-2 border-[#070A14] flex items-center justify-center text-xs font-black text-blue-400">
                  +{members.length - 3}
                </div>
              )}
            </div>
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover/btn:text-blue-400 transition-colors">Unit Manager</span>
          </button>
        </div>
      </header>
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-8">
                ...
                <div className="text-right">
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{category.count} holdings</p>
              ...
              <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{category.docCount} Proofs</span>
      ...
      {/* Modern Overlay Modals Styled Glass */}
      {showMemberManager && (
        <FamilyMemberManager 
          familyId={familyId} 
        ...
        <div className="fixed inset-0 bg-[#070A14]/80 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="glass rounded-[48px] border-white/10 w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl shadow-blue-950/20">
            <div className="p-10 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
              <div>
                <h3 className="text-4xl font-black text-white tracking-tighter">{activeCategory}</h3>
                <p className="text-slate-400 font-medium uppercase tracking-widest text-[10px] mt-2">Managing records for: <span className="text-white">{selectedMemberId === 'ALL' ? 'Family Entity' : members.find(m => m.id === selectedMemberId)?.name}</span></p>
              </div>
            ...
            <div className="flex-1 overflow-auto p-10 space-y-12 custom-scrollbar">
               <div className="glass p-8 rounded-[32px] border-white/5">
                  <h4 className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-8">Data Entry Node</h4>
                  <AssetForm 
                    familyId={familyId}
                    members={members}
               ...
               <div className="space-y-8">
                  <div className="flex items-center justify-between border-b border-white/5 pb-6">
                    <h4 className="text-2xl font-black text-white tracking-tight">Active Holdings</h4>
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-white/5 px-4 py-1.5 rounded-full">{currentAssets.filter(a => a.type === activeCategory).length} Nodes</span>
                  </div>
                    ...
                    {currentAssets.filter(a => a.type === activeCategory).map(asset => (
                      <div key={asset.id} className="glass p-6 rounded-[24px] border-white/5 hover:border-white/10 transition-all group/card">
                        <div className="flex justify-between items-start mb-6">
                           ...
                           <button onClick={() => { deleteAsset(asset.id); setRefresh(r => r + 1); }} className="text-slate-600 hover:text-red-400 p-2 rounded-xl hover:bg-red-500/10 transition-all opacity-0 group-hover/card:opacity-100">🗑️</button>
                        </div>
